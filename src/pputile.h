...
if (X1 >= 1) 
{
	if ((scanline < 240) && (scanline & 7) == 1)
	{
		int i = X1 - 1;
		tile.palette = cc;
		tile.x = (i << 3) - XOffset;
		tile.y = scanline - (vofs & 7);
		tile.flipY = tile.flipX = false;
		tile.index = (scanline >> 3) * WIDTH + i;
		tile.isBg = true;
...
pshift[1] <<= 8;
	...
	tile.low = *(int64_t*)(C - (vofs & 7));
	tile.high = *(int64_t*)(C + 8 - (vofs & 7));
