...
int32_t tile_count;
TileData tile_buffer[SIZE];

bool empty(TileData* data)
{
    return (data->low == 0) && (data->high == 0);
};
...
void RenderSprite()
{
   ...
   int segment;
	...
	if ((scanline & offsetMask) != offset)
		return;

	spr = (SPR*)SPRAM;
	tile.y = spr->y + 1;
	vofs = (uint32)(P0 & 0x8 & (((P0 & 0x20) ^ 0x20) >> 2)) << 9;
	segment = scanline & segmentMask;
	
	for (n = 63; n >= 0; n--, spr++, tile.y = spr->y + 1) 
	if (((tile.y & segmentMask) == segment) && (tile.y < 240))
	{
		if (Sprite16)
		{
			if (spr->atr & V_FLIP)
				vadr = ((spr->no & 1) << 12) + ((spr->no | 0x01) << 4);
			else
		...
		if (MMC5Hack && geniestage != 1)
			C = (int64_t *)MMC5SPRVRAMADR(vadr);
		else
			C = (int64_t *)VRAMADR(vadr);

		tile.low = *C;
		tile.high = *(C + 1);
		tile.x = spr->x;
		//tile.y = spr->y + 1;
		tile.hidden = spr->atr & SP_BACK;
		tile.flipY = spr->atr & V_FLIP;
		tile.flipX = spr->atr & H_FLIP;
		tile.palette = spr->atr & 3;
		tile.index = n;

		//if (!empty(&tile))
			tile_buffer[tile_count++] = tile;
